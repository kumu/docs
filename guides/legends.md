...
2. Using the [decoration builders](decorate.html#refine-your-decorations)
...
Using Kumu's [computed fields](computed-fields.html) feature, you can quickly solve this problem and create a legend with only the used values:
...
For bonus points, use [field relevance settings](fields.html#field-relevance) to hide your computed field from all profiles.

#### Use the decoration builders to add legend entries

To add custom colors and icons to your legend, use the [element and connection decoration builders](decorate.html#refine-your-decorations). Once you build out your decoration rule, fill in the "Label for Legend" box at the bottom of the decoration builder, and Kumu will add your decoration to the legend.
...
Some legend entries get added automatically when you apply different [templates](templates.html). Here's how to delete those entries:

* For **Person** and **Organization** entries (stakeholder template), and **Stock**, **Flow**, **Variable**, **Source**, and **Sink** entries (stock and flow template), follow the guide to [delete field values](fields.html#deleting-field-values). Delete these values from the Element Type field.
* For **Adds to / same direction** and **Subtracts from / opposite direction** entries (causal loop template), follow the guide to [delete field values](fields.html#deleting-field-values). Delete these values from the Connection Type field.
* **Opposite** entry (system template): Open your [default settings](default-view-settings.html), and, in the **Template settings** section, clear out the "Opposite connection label" box and click **SAVE** at the bottom.
...
1. Click the menu icon to bring up the [project settings](../overview/settings.html#project-settings)
...
For more info on why and how this works, check out [our full guide on built-in controls](controls.html#built-in-controls).
